...
pub enum QueryStreamRequest {
    ...
    Next {
        ...
        resp_tx: oneshot::Sender<LensResult<Option<Vec<model::Row>>>>,
    },
}
...
impl QueryStreamRequest {
    ...
    pub fn next(id: StreamId) -> (Self, oneshot::Receiver<LensResult<Option<Vec<model::Row>>>>) {
        let (resp_tx, resp_rx) = oneshot::channel();
...
impl QueryStreamer {
    ...
    async fn stream_next(&mut self, id: StreamId) -> StreamResult<Option<Vec<model::Row>>> {
